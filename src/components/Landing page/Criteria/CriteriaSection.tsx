...
export default function CriteriaSection() {
  return (
    <section className="py-20 bg-[rgba(255,255,255,0.95)]" id="criteria">
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div>
            <h2 className="text-4xl font-bold text-[var(--hover)] mb-4">
              Selection Criteria
            </h2>
            <p className="text-lg text-[oklch(0.45_0.03_250)] mb-6">
          ...
          <div className="space-y-4">
            {criteria.map((item, index) => (
              <div key={index} className="flex items-start">
                <CheckCircle2 className="w-6 h-6 text-[var(--accent)] flex-shrink-0 mt-1 mr-3" />
                <span className="text-[var(--hover)] text-lg">{item}</span>
              </div>
            ))}
          </div>
        </div>
