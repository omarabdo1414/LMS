    ...
    <section className="py-20" id="benefits">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-[var(--hover)]">Why Choose Us?</h2>
          <p className="text-lg text-[oklch(0.45_0.03_250)] mt-2">
            The best features to help you learn and grow.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 cursor-pointer">
          {features.map((feature, index) => (
            <div
              key={index}
              className="bg-card p-8 rounded-lg border border-border text-center transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
            >
              <div className="flex justify-center items-center mb-4">{feature.icon}</div>
              <h3 className="text-xl font-bold text-[var(--hover)] mb-2">{feature.title}</h3>
              <p className="text-[oklch(0.45_0.03_250)]">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
