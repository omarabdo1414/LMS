...
async function fetchExams(): Promise<Exam[]> {
  ...
  return data; // Return the raw response to see its structure
}
...
export default function Page() {
  ...
  const examsList = Array.isArray(exams) ? exams : exams?.data || exams?.exams || [];
            ...
            <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow-sm">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {examsList.map((exam: any, index: number) => (
                  <tr key={exam.id || exam._id || index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <button
                        onClick={() => handleStartExam(exam.id || exam._id || index.toString())}
